...
def quality_eval(question: str, answer: str) -> int:
    """Evaluate a single answer as correct (1) or incorrect (0) for Cohen κ calculation."""
    prompt = (
        f"Câu hỏi: {question}\n\n"
        f"Câu trả lời cần đánh giá: {answer}\n\n"
        "Đánh giá câu trả lời này là ĐÚNG (đủ thông tin, không sai sự kiện) hay SAI (thiếu thông tin quan trọng hoặc sai sự kiện).\n"
